...
$ with maps $A_n \to A_\ell$ and $A_m \to A_\ell$, which gives us the following (noncommutative) diagram of maps. Composing these maps with $g$ and $f'$, this gives two maps $A_j \to A_\ell$, which guarantees the existence of a map $A_\ell \to A'_\ell$ whose compositions with the two maps $A_j \to A_\ell$ are the same.
$$\begin{tikzcd}
...
& & A'_\ell & &
\end{tikzcd}$$
Then since $f(a_i) = g(a_j)$ and $f'(a_j) = f'(a_k)$, and the maps $A_j \to A_n \to A_\ell \to A'_\ell$ and $A_j \to A_m \to A_\ell \to A'_e\ll$ are the same, we have that the compositions $A_i \to A'_ell$ and $A_k \to A'_\ell$ map $a_i$ and $a_k$ to the same place, so we can finally conclude that $(a_i,i) \sim (a_k,k)$.
