...
$$\xymatrix{
\hom(C,A) \ar[d] \ar[r]^{i_A} & \hom(C,A') \ar[d] \\
\hom(B,A) \ar[r]_{i_A} & \hom(B,A')
}$$
Chasing the diagram reveals that the diagram commutes, since any map $\varphi \in \hom(C,A)$ gets mapped to $i_A(\id_A) \circ \varphi \circ f \in \hom(B,A')$. We now want to show that this map is unique.
\end{enumerate}
