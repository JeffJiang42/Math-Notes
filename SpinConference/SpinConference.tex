...
\end{tikzcd}$$
\begin{enumerate}
\item Consider the map $j : V \to \mathcal{T}(V) / (v^2 - 2b(v,v))$ induced by the inclusion map $V \hookrightarrow \mathcal{T}(V)$.  We note that this map is injective, and satisfies the relation we need to get a map $\varphi : \Cliff(V,b) \to \mathcal{T}(V) / (v^2 - 2b(v,v)$ such that $j = \varphi \circ i$. Therefore, $i$ must be injective.
\item 
\end{enumerate}
%
\begin{exer}
