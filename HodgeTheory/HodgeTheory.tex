...
\[\begin{tikzcd}
\Lambda^{k-2}V \ar[r, bend left=30, "L"] & \Lambda^kV \ar[l, bend left=30, "\Lambda"]
...
There is an orthgonal direct sum decomposition
\[
\Omega^k_X = \bigoplus_{\ell \geq 0 } L^iP^{k-2i}
\]
\end{thm}
%
\begin{proof}
Since $L$ is order $0$, it suffices to verify this fiberwise. Let $V = T^*_xX$.
...
\end{proof}
%
We then note that $\Lambda^\bullet V$ admits a primitive $\alpha$, since we can just
pick a nonzero $\alpha$, and walk it down the chain by repeatedly applying $\Lambda$,
...
\iffalse
We now prove another corollary of Proposition 8.1.
%
\begin{prop}
The map $L^{n-k} : \Lambda^kT^*M \to \Lambda^{2n-k}T^*K$ is an isomorphism.
\end{prop}
%
\begin{proof}
We first prove some lemmas
...
\end{proof}
%
we the prove the proposition with a bit of $\sl_2\C$ representation theory. It
suffices to prove that $L^{n-k}$ is an isomorphism fiberwise. Let $V = T^*_xX$.
Since $\Lambda^\bullet V$ is finite dimensional, we know that there must exist a
non-zero element $v \in \Lambda^\bullet V$ such that $\Lambda v = 0$. Indeed, if
we start with a vector $w$, the sequence $\Lambda w, \Lambda^2 w, \ldots $ must
terminate at some finite $n$, since all of the vectors are linearly independent,
since they are eigenvectors of $\Pi$ of different eigenvalues. From $\sl_2\C$
representation theory (or a somewhat quick calculuation), we have that the set
$\set{v, Lv, L^2v, \ldots }$ spans a subrepresentation of $\Lambda^\bullet V$.
In addition, in the decomposition of this subrepresentation  into eigenspaces of $\Pi$,
it is clear that each eigenspace is $1$-dimensional (each one is spanned by $L^kv$),
so it is an irreducible $\sl_2\C$ representation.
With these lemmas in hand, we note that if $L^r$ annihilates
$\alpha \in \Omega^k_X$, we have that
\begin{align*}
&L^r\Lambda\alpha = [L^r,\Lambda]\alpha - \Lambda L^2\alpha \\
\implies &L^r\Lambda\alpha - [L^r,\Lambda]\alpha = 0
\end{align*}
and we compute, using our first lemma,
\begin{align*}
L^r\Lambda\alpha - [L^r,\Lambda]\alpha &= L^{r-1}(L\Lambda\alpha)
- (r(k-n) + r(r-1))L^{r-1}\alpha\\
&= L^{r-1}(L\Lambda - (r(k-n)+r(r-1))\id)\alpha
\end{align*}
